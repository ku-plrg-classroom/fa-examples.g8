...
case class ENFA(
  ...
  /** The safe read of the transition function of ENFA */
  def getTrans(q: State, a: Option[Symbol]): Set[State] =
  ...
  def eclo(q: State): Set[State] =
    def aux(rest: List[State], visited: Set[State]): Set[State] = rest match
      ...
      case p :: targets =>
        aux(
          rest = (getTrans(p, None) -- visited - p).toList ++ targets,
          visited = visited + p,
        )
    aux(List(q), Set())
  ...
  def extTrans(qs: Set[State], w: Word): Set[State] = w match
    case ""     => eclo(qs)
    case x <| w => extTrans(eclo(qs).flatMap(q => getTrans(q, Some(x))), w)
...
object ENFA:
  /** Create a new ENFA from transition pairs (varargs)
    *
    * Supports both regular symbol transitions (`Some(symbol)`) and epsilon
    * transitions (`None`). Automatically merges duplicate keys into a Set of
    * target states.
    *
    * @param states
    *   the set of states
    ...
    *   the set of final states
    * @param pairs
    *   the transition pairs (symbol is `Option[Symbol]`)
    * @return
    *   a new ENFA
    */
  def apply(
    ...
    initState: State,
    finalStates: Set[State],
  )(pairs: ((State, Symbol | EPS), State)*): ENFA =
    val trans = pairs.groupBy(_._1).map {
      case ((state, symbol), values) =>
        val opt: Option[Symbol] = symbol match
          case a: Symbol => Some(a)
          case _: EPS    => None
        (state, opt) -> values.map(_._2).toSet
    }
    ENFA(states, symbols, trans, initState, finalStates)
