...
trait FA extends Acceptable {
  ...
  /** The number of transitions in the finite automaton */
  lazy val size: Int = this match
    case dfa: DFA   => dfa.trans.size
    case nfa: NFA   => nfa.trans.map(_._2.size).sum
    case enfa: ENFA => enfa.trans.map(_._2.size).sum

  /** Checks if the number of transitions is less than or equal to the given
    * bound, or throws an exception
    */
  def <=(bound: Int): this.type =
    if (size > bound) error(s"Too many transitions: $size > $bound")
    this
